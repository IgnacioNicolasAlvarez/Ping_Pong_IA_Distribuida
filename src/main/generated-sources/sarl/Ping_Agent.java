...
public class Ping_Agent extends Agent {
  private Integer count;
  
  private Double performance;
  
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    this.performance = Double.valueOf(Math.random());
    Logging _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER = this.$castSkill(Logging.class, (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING == null || this.$CAPACITY_USE$IO_SARL_CORE_LOGGING.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING = this.$getSkill(Logging.class)) : this.$CAPACITY_USE$IO_SARL_CORE_LOGGING);
    _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info(("AgentePint Performance: " + this.performance));
  ...
  private void $behaviorUnit$Pong$1(final Pong occurrence) {
    Schedules _$CAPACITY_USE$IO_SARL_CORE_SCHEDULES$CALLER = this.$castSkill(Schedules.class, (this.$CAPACITY_USE$IO_SARL_CORE_SCHEDULES == null || this.$CAPACITY_USE$IO_SARL_CORE_SCHEDULES.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_SCHEDULES = this.$getSkill(Schedules.class)) : this.$CAPACITY_USE$IO_SARL_CORE_SCHEDULES);
    final Procedure1<Agent> _function = (Agent it) -> {
      InputOutput.<String>println(("send ping " + this.count));
      DefaultContextInteractions _$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS$CALLER = this.$castSkill(DefaultContextInteractions.class, (this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS == null || this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS = this.$getSkill(DefaultContextInteractions.class)) : this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS);
      Ping _ping = new Ping(this.count);
      _$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS$CALLER.emit(_ping);
      this.count = Integer.valueOf((((this.count) == null ? 0 : (this.count).intValue()) + 1));
    };
    _$CAPACITY_USE$IO_SARL_CORE_SCHEDULES$CALLER.in(1000, _function);
  }
  
  protected Object sendPing() {
    ...
    if ((_size > 1)) {
      Integer _xblockexpression = null;
      {
        ...
        _xblockexpression = this.count = Integer.valueOf((((this.count) == null ? 0 : (this.count).intValue()) + 1));
      }
  ...
  private void $guardEvaluator$Pong(final Pong occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$Pong$1(occurrence));
  }
  ...
  public boolean equals(final Object obj) {
    ...
    if (getClass() != obj.getClass())
      return false;
    Ping_Agent other = (Ping_Agent) obj;
    if (other.count != this.count)
      return false;
    if (Double.doubleToLongBits(other.performance) != Double.doubleToLongBits(this.performance))
      return false;
  ...
  public int hashCode() {
    ...
    result = prime * result + (int) (Double.doubleToLongBits(this.performance) ^ (Double.doubleToLongBits(this.performance) >>> 32));
    return result;
  }
  
  @SyntheticMember
  public Ping_Agent(final UUID parentID, final UUID agentID) {
  ...
  @Inject
  @Deprecated
  public Ping_Agent(final BuiltinCapacitiesProvider provider, final UUID parentID, final UUID agentID) {
    super(provider, parentID, agentID);
  }
  
  @SyntheticMember
  @Inject
  public Ping_Agent(final UUID parentID, final UUID agentID, final DynamicSkillProvider skillProvider) {
